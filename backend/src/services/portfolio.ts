...
export class PortfolioService {
  ...
  ): Promise<ApiResponse<PortfolioWithCoin[]>> {
    try {
  ...
  async addOrUpdateHolding(
    ...
    try {
      ...
      if (existingHolding) {
        ...
        const totalValue =
          Number(existingHolding.amount) * Number(existingHolding.avgPrice) +
  ...
  async getPortfolioAnalytics(
    ...
    try {
      ...
      const sortedByPerformance = [...holdingsWithPerformance].sort(
        (a, b) => Number(b.profitLossPercentage) - Number(a.profitLossPercentage)
      );
          ...
          for (const holding of walletHoldings) {
            try {
              ...
              let coin = await this.prisma.coin.findFirst({
                where: {
                  OR: [{ address: holding.tokenAddress }, { symbol: holding.symbol }],
                  network,
                },
              });
   ...
   */
  private async getWalletHoldings(
